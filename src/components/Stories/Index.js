...
const Stories = ({stories, isOpenStory, setIsOpenStory}) => {
    ...
    const viewThisStory = (index) => {
        ...
        axios.get(`api/v1/post/viewStory/${storyId}`,
            {headers: GetHeaders()})
        .then(res => {
    ...
    let storyList = stories.map((story, index) => {
        // console.log(story)
        return (
            <div key={story._id} className="cursor-pointer  flex flex-col items-center ml-4 pt-3 pb-2">
                <div onClick={() => openStory(story, index)}> 
                    <div className={`${story.seen?"rounded-corners-gray-borders":"rounded-corners-gradient-borders"} rounded-full`}>
                        <img src={story.image} className="w-16 h-16 rounded-full border-2 border-white" alt="story img"/>
                    </div>
                </div>
                ...
                {isOpenStory && <Story story={stories[currentStoryIndex]} prevImage={prevImage} nextImage={nextImage} closeStory={closeStory}/> }
            </div>
        )
    })
    ...
    return (
        <div>
                ...
                <div className="relative max-w-xl h-28 sm:mt-7">
                    <div ref={scrollableDiv} className="stories max-w-xl bg-white border border-insta-border-gray h-28 flex flex-row overflow-auto">
                        {storyList}
                    </div>
                    {currentScrollPos !== 0 && (
                        <div
